...
### Authentication Handling with `createAuthProvider`
The recent updates introduce a new way to handle authentication using the `createAuthProvider` function. This function allows you to configure different types of authentication, such as Basic and OAuth2, for your API connections.

#### Setting Up Authentication
To set up authentication, you need to configure the `ApiClient` with the appropriate authentication type and credentials. The `createAuthProvider` function will return an instance of the specified authentication provider.
...
The `OAuth2Provider` supports various OAuth2 flows, including authorization code and client credentials. It handles token acquisition and refresh automatically.

- **Authorization URL**: Use `getAuthorizationUrl` to generate the URL for user authorization.
- **Access Token**: Use `getAccessToken` to exchange an authorization code for an access token.
- **Token Refresh**: Use `refreshToken` to refresh an expired access token using a refresh token.

This setup ensures that your API requests are authenticated and authorized according to the specified configuration.
```